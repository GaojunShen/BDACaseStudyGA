...
print(m1,'chart')
```
The top 5 pages with more visits are 
```r
p1 = pageviewsppage[1:5,]  #create p1 to record top 5 pages data
```
...
print(m1,'chart')
```
When the Medium is not "cpc", the top 3 pages with more visits are 
```r
p2 = pageviewsppage[1:5,] #p2 means is not cpc and record top 5 pages data
```
They are not the same, the number 3 page changes.
Furthermore, the second place is a "/" which does not provide any information. 
Need to improve algorithm and screen out useless information to provide more accurate data.
...
print(m1,'chart')
```
Top 5 pages with most exits are `r exitppage[1:5,]`.Their pageviews are `r pageviewsppage[1:5,]` respectively. So their exit rates are `r exitppage[1:5,]/pageviewsppage[1:5,]`.

To reduce exit rate, we can reduce the number of exits and/or increase the number of pageviews. We can re-design the page layout so that visitors can easily find the information they need to achieve this goal.
...
```
```r
p3 <- pageviewsppage - exitppage    #to get the difference between pageviews and exits
p4 <- timeppage/p3    #to get the average time on page
p4[1:3,]       #to get the top 3 with highest average time on page
```
<br>
...
source_mediumppage[source_mediumppage$users = "New Visitors",]
<br>
...
print(m1,'chart')
```
When medium is referral, the pages with highest pageviews
```r
source_referral[1:3,]   #top 3 number of pageviews when referral
```

When medium is cpc, the page with highest pageviews
```{r echo=TRUE, eval=TRUE, tidy=TRUE}
gadata_cpc = gadata[gadata$Medium =="cpc", ]
source_cpc <-  group_by(gadata_cpc, factor(source)) %>% summarise(pageviews = sum(pageviews))
source_cpc = data.frame(pages = source_cpc$`factor(source)`, pageviews = source_cpc$pageviews)
source_cpc = source_cpc[sort(source_cpc$pageviews, decreasing =  TRUE, index.return = TRUE)$ix,]
```
```
source_cpc[1:3,]    #top 3 numer of pageviews when cpc
```

When medium is referral, it is hard to say if the author should promote the content because it is hard to compare with available information. When medium is cpc (paid search engine), the only source is google and it is hard compare either.